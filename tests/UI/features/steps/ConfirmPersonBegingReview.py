...
def step_impl(context):
    ...
    driver.enterValues(searchPerson, 'Brittany Beck')
...
def step_impl(context):
    ...
    assert "Brittany Beck" in chkModal[counter].text, "Modal Box is not displaying correctly"

@step("the correct Unit/Department is selected")
def step_impl(context):
    time.sleep(2)
   # driver.waitOnElement(checkTerms)
   # driver.elementClick(checkTerms)
    time.sleep(2)
